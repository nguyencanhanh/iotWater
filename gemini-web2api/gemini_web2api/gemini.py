                    ...
                    while "\n" in buf:
                        ...
                        for t in _extract_texts_from_line(line):
                            if len(t) > len(prev_text):
                                prev_text = t
                                # Phai clean tren toan bo van ban roi moi cat delta.
                                # Clean tung mieng se lam .strip() an mat dau cach
                                # va xuong dong o ranh gioi giua cac chunk.
                                cleaned = clean_text(t)
                                if len(cleaned) > len(prev_clean):
                                    delta = cleaned[len(prev_clean):]
                                    prev_clean = cleaned
                                    if delta:
                                        yield delta
            return
